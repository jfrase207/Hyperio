...
void main() {
	...
	float dist = sqrt((playerPos.x-zposasteroid.x) * (playerPos.x-zposasteroid.x) + (playerPos.y-zposasteroid.y) * (playerPos.y-zposasteroid.y) + (playerPos.z-zposasteroid.z) * (playerPos.z-zposasteroid.z));
		

	float fogFactor = (maxDist - dist) / (maxDist - minDist);
	fogFactor = clamp( fogFactor, 0.0, 1.0 );
	vec3 fog = Fog(); // this is your method from above

	vec3 color = mix( fogColor, fog, fogFactor);
